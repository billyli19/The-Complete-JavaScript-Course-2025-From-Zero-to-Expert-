...
const formatMovementDate = function (date, locale) {
  ...
  const daysPassed = calcDaysPassed(new Date(), validDate);

  if (daysPassed === 0) return 'Today';
  if (daysPassed === 1) return 'Yesterday';
  if (daysPassed <= 7) return `${daysPassed} days ago`;

  return new Intl.DateTimeFormat(currentAccount.locale).format(validDate);
};
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    // Each call will print the remaining time to UI
    labelTimer.textContent = formattedTimer;
